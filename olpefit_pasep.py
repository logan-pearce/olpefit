...
prepost = raw_input("Was this observation pre or post April 13, 2105? Type 'pre' or 'post' ")
...
minxy = min(len(xcs[0]),len(ycs[0]),len(xcc[0]),len(ycc[0]))
xcs = [xcs[i][:minxy] for i in range(NWalkers)] 
ycs = [ycs[i][:minxy] for i in range(NWalkers)]
xcc = [xcc[i][:minxy] for i in range(NWalkers)] 
ycc = [ycc[i][:minxy] for i in range(NWalkers)]
...
sep = [r[i]*pixscale for i in range(NWalkers)] #converted to arcseconds
...
directory = filename.split('/')[0]+'/'+filename.split('/')[1]+'/epoch_positions_olpefit'
...
xcc = [xcc[i][:minxy] for i in range(NWalkers)] 
ycc = [ycc[i][:minxy] for i in range(NWalkers)]
amps = [amps[i][:minxy] for i in range(NWalkers)]
ampc = [ampc[i][:minxy] for i in range(NWalkers)]
ampratio = [ampratio[i][:minxy] for i in range(NWalkers)]
bkgd = [bkgd[i][:minxy] for i in range(NWalkers)]
sigmax = [sigmax[i][:minxy] for i in range(NWalkers)]
sigmay = [sigmay[i][:minxy] for i in range(NWalkers)]
sigmax2 = [sigmax2[i][:minxy] for i in range(NWalkers)]
sigmay2 = [sigmay2[i][:minxy] for i in range(NWalkers)]
theta = [theta[i][:minxy] for i in range(NWalkers)]
theta2 = [theta2[i][:minxy] for i in range(NWalkers)]
...
cornerplot = filename.split('/')[0]+'/'+filename.split('/')[1]+'/'+filename.split('.')[2]+ '_olpefit_results'
